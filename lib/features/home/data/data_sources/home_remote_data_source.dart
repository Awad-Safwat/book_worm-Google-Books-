...
class HomeRemoteDataSourceImpl extends HomeRemoteDataSource {
  ...
  Future<List<BookEntity>> fetchFeatueredBooks({int pageNumber = 0}) async {
    var response = await apiService.get(
        endPoint:
            'volumes?q=history&filter=ebooks&orderBy=relevance&startIndex=${pageNumber * 10}');
  ...
  Future<List<BookEntity>> fetchNewestBooks({int pageNumber = 0}) async {
    var response = await apiService.get(
        endPoint:
            'volumes?q=programming&filter=ebooks&orderBy=relevance&startIndex=${pageNumber * 10}');
